...
//This is for model3 Alpha-Beta Pruning

//the simulation function for the branches in the searching tree
int ai_model3_AB_simulate(GameState *gameState, Player *player,
    int alpha, int beta, int depth)
{
    ...
    for(int i=0;i<total_num_moves;i++)
    {
        #pragma omp task shared(Scores)
        {
            ...
            int score=playerTurn*ai_model3_AB_simulate(&simulation,player,
                alpha, beta, depth-1);
            Scores[i]=score;
...
int ai_model3_AB_play(GameState *gameState, Player *player, int maxStep)
{
    ...
    for(int i=0;i<total_num_moves;i++)
    {
        ...
        score=playerTurn*ai_model3_AB_simulate(&simulation,player,
            -60000,60000,maxStep);
        Scores[i]=score;
        env_free_state(&simulation);
    }
